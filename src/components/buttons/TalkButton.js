...
const TalkButton = ({ name = "Let's talk" }) => {
  // Add name prop with default value
  ...
  useEffect(() => {
    ...
    const magneticLoop = () => {
      ...
      gsap.set(content, { x: buttonX * 0.2, y: buttonY * 0.2 });
    ...
    const mouseMove = (e) => {
      ...
      if (Math.random() > 0.8) createParticle(clientX, clientY);
    };
    ...
    const mouseEnter = () => {
      ...
      setIsHovered(true);
      magneticLoop();
      gsap.to(content, { scale: 1.04, duration: 0.3, ease: "power2.out" });
    ...
    <a href='#contacts' style={{ textDecoration: "none" }}>
      <div
        ...
        <div
          ...
          <p className='btnText'>{name}</p> {/* Use the name prop here */}
          <svg
            className='arrow'
